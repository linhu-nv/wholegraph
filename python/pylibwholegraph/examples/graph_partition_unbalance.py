...
def build_partitioned_subgraphs(args, new2origin_node_map):
    ...
    for i in range(num_part):  # scan all nodes, collect already-in-right-rank nodes and extra nodes
        if i == num_part - 1:
            std_node_num = node_num - (num_part - 1) * rank_node_num
        else:
            std_node_num = rank_node_num
        ...
        for j in range(my_rank_start, my_rank_end):
            ...
            if real_ranks_nodes[i] >= std_node_num:
                extra_nodes.append(origin_id)
            else:
                real_ranks_nodes[i] += 1
    ...
    for i in range(num_part):  # fullfill those unfilled ranks with extra nodes
        if i == num_part - 1:
            ...
            std_node_num = rank_node_num
        needed_nodes = std_node_num - real_ranks_nodes[i]
        if needed_nodes > 0:
            assert len(extra_nodes) >= needed_nodes
            ranks_node_id[i] += extra_nodes[:needed_nodes]
            extra_nodes = extra_nodes[needed_nodes:]
