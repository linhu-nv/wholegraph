...
def test(test_dataset, model):
    test_dataloader = wgth.get_valid_test_dataloader_partitioned(
        test_dataset,
        options.batchsize
    )
...
def train(train_data, valid_data, model, optimizer, wm_optimizer, global_comm):
    ...
    train_dataloader = wgth.get_train_dataloader_partitioned(
        train_data,
        options.batchsize,
        num_workers=options.dataloaderworkers,
    )
    valid_dataloader = wgth.get_valid_test_dataloader_partitioned(
        valid_data,
        options.batchsize)
    ...
    print("rank ", wgth.get_rank(), " has ", len(train_dataloader), "steps")
...
def main_func():
    ...
    print("total world rank size is ", wgth.get_world_size())
    if options.use_cpp_ext:
        wgth.compile_cpp_extension()

    train_ds, valid_ds, test_ds = wgth.create_node_claffication_datasets_partitioned(
        options.pickle_data_path, wgth.get_rank()
    )
